...
$(document).ready(function() {
    ...
    function getIcon(data) {
        var path = "";
        if (200 < data.weather[0].id && data.weather[0].id < 300) {
            path = "img/thunder.png";
            return path;
        } else if (300 <= data.weather[0].id && data.weather[0].id < 400) {
            path = "img/drizzle.png";
            return path;
        } else if (500 <= data.weather[0].id && data.weather[0].id < 600) {
            path = "img/rain.png";
            return path;
        } else if (600 <= data.weather[0].id && data.weather[0].id < 700) {
            path = "img/snow.png";
            return path;
        } else if (700 <= data.weather[0].id && data.weather[0].id < 800) {
            path = "img/atmosphere.png";
            return path;
        } else if (data.weather[0].id == 800) {
            path = "img/clear.png";
            return path;
        } else if (801 <= data.weather[0].id && data.weather[0].id <= 804) {
            path = "img/cloud.png";
            return path;
        } else if (900 <= data.weather[0].id && data.weather[0].id <= 906) {
            path = "img/extreme.png";
            return path;
        } else if (951 <= data.weather[0].id && data.weather[0].id <= 962) {
